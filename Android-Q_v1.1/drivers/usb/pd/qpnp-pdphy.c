...
struct usb_pdphy {
	...
	void (*msg_rx_cb)(struct usbpd *pd, enum pd_sop_type sop, u8 *buf, size_t len);
...
static int pdphy_dbg_status(struct seq_file *s, void *p)
{
	...
	seq_printf(s,"PD Phy driver status\n"	"==================================================\n");
...
static void pdphy_create_debugfs_entries(struct usb_pdphy *pdphy)
{
	...
	ent = debugfs_create_file("status", 0400, pdphy->debug_root, pdphy, &status_ops);
	if (!ent) {
...
static int pdphy_enable_power(struct usb_pdphy *pdphy, bool on)
{
	...
	ret = regulator_set_voltage(pdphy->vdd_pdphy, VDD_PDPHY_VOL_MIN, VDD_PDPHY_VOL_MAX);
	if (ret) {
		dev_err(pdphy->dev,	"set voltage failed for vdd_pdphy:%d\n", ret);
...
unset_pdphy_vdd:
	...
	if (ret)
		dev_err(pdphy->dev,	"Unable to set (0) voltage for vdd_pdphy:%d\n", ret);
...
void pdphy_enable_irq(struct usb_pdphy *pdphy, bool enable)
{
	if (enable) {
		...
		pdphy->sig_rx_wake_enabled =!enable_irq_wake(pdphy->sig_rx_irq);
		enable_irq(pdphy->msg_tx_irq);
		if (!pdphy->in_test_data_mode) {
			enable_irq(pdphy->msg_rx_irq);
			pdphy->msg_rx_wake_enabled =!enable_irq_wake(pdphy->msg_rx_irq);
		}
...
static int pdphy_bulk_reg_write(struct usb_pdphy *pdphy, u16 addr, const void *val, u8 val_cnt)
{
	int ret;

	ret = regmap_bulk_write(pdphy->regmap, pdphy->base + addr, val, val_cnt);
	if (ret) {
...
static int pdphy_masked_write(struct usb_pdphy *pdphy, u16 addr, u8 mask, u8 val)
{
	int ret;
...
int pd_phy_open(struct pd_phy_params *params)
{
	...
	/* PD 2.0  phy */
	ret = pdphy_masked_write(pdphy, USB_PDPHY_MSG_CONFIG,MSG_CONFIG_SPEC_REV_MASK, USBPD_REV_20);
	if (ret)
		return ret;
	...
	/* update frame filter */
	ret = pdphy_reg_write(pdphy, USB_PDPHY_FRAME_FILTER,pdphy->frame_filter_val);
	if (ret)
		return ret;
...
int pd_phy_signal(enum pd_sig_type sig)
{
	...
	val = (sig == CABLE_RESET_SIG ? TX_CONTROL_FRAME_TYPE_CABLE_RESET : 0)	| TX_CONTROL_SEND_SIGNAL;

	ret = pdphy_reg_write(pdphy, USB_PDPHY_TX_CONTROL, val);
	if (ret)
		return ret;

	ret = wait_event_interruptible_hrtimeout(pdphy->tx_waitq,pdphy->tx_status != -EINPROGRESS,	ms_to_ktime(HARD_RESET_COMPLETE_TIME));
	if (ret) {
...
int pd_phy_write(u16 hdr, const u8 *data, size_t data_len, enum pd_sop_type sop)
{
	...
	ret = pdphy_bulk_reg_write(pdphy, USB_PDPHY_TX_BUFFER_HDR, (u8 *)&hdr,	USB_PDPHY_MSG_HDR_LEN);
	if (ret)
		return ret;

	if (data && data_len) {
		print_hex_dump_debug("tx data obj:", DUMP_PREFIX_NONE, 32, 4,data, data_len, false);

		/* write data objects of SOP message */
		ret = pdphy_bulk_reg_write(pdphy, USB_PDPHY_TX_BUFFER_DATA,data, data_len);
		if (ret)
			return ret;
	}
	...
	ret = wait_event_interruptible_hrtimeout(pdphy->tx_waitq,pdphy->tx_status != -EINPROGRESS,ms_to_ktime(RECEIVER_RESPONSE_TIME));
	if (ret) {
		dev_err(pdphy->dev, "%s: failed ret %d\n", __func__, ret);
		return ret;
	}
...
static int pd_phy_bist_mode(u8 bist_mode)
{
	...
	return pdphy_masked_write(pdphy, USB_PDPHY_BIST_MODE,	BIST_MODE_MASK | BIST_ENABLE, bist_mode | BIST_ENABLE);
}
...
static irqreturn_t pdphy_msg_rx_irq(int irq, void *data)
{
	...
	print_hex_dump_debug("rx msg:", DUMP_PREFIX_NONE, 32, 4, buf, size + 1,	false);
	pdphy->rx_bytes += size + 1;
done:
	return IRQ_HANDLED;
}

static int pdphy_request_irq(struct usb_pdphy *pdphy,struct device_node *node,
				...
				int flags)
{
	...
	*irq_num = of_irq_get_byname(node, irq_name);
	...
	ret = devm_request_threaded_irq(pdphy->dev, *irq_num, irq_handler,	thread_fn, flags, irq_name, pdphy);
	if (ret < 0) {
...
static int pdphy_probe(struct platform_device *pdev)
{
	...
	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->sig_tx_irq, "sig-tx", NULL,
		pdphy_sig_tx_irq_thread, (IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->sig_rx_irq, "sig-rx", NULL,
		pdphy_sig_rx_irq_thread, (IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->msg_tx_irq, "msg-tx", pdphy_msg_tx_irq,
		NULL, (IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->msg_rx_irq, "msg-rx", pdphy_msg_rx_irq,
		NULL, (IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->msg_tx_failed_irq, "msg-tx-failed", pdphy_msg_tx_irq,
		NULL, (IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->msg_tx_discarded_irq, "msg-tx-discarded",
		pdphy_msg_tx_irq, NULL,
		(IRQF_TRIGGER_RISING | IRQF_ONESHOT));
	if (ret < 0)
		return ret;

	ret = pdphy_request_irq(pdphy, pdev->dev.of_node,&pdphy->msg_rx_discarded_irq, "msg-rx-discarded",
