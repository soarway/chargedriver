...
#define usbpd_dbg(dev, fmt, ...) do { \
	ipc_log_string(usbpd_ipc_log, "%s: %s: " fmt, dev_name(dev), __func__, ##__VA_ARGS__); \
...
#define PD_SNK_PDO_FIXED(prs, hc, uc, usb_comm, drs, volt, curr) \
	(((prs) << 29) | ((hc) << 28) | ((uc) << 27) | ((usb_comm) << 26) | \
	 ((drs) << 25) | ((volt) << 10) | (curr))
#define PD_SNK_PDO_FIXED_MAX_CURR(pdo)		PD_SRC_PDO_FIXED_MAX_CURR(pdo)
#define PD_SNK_PDO_FIXED_VOLTAGE(pdo)		PD_SRC_PDO_FIXED_VOLTAGE(pdo)
...
static const u32 default_src_caps[] = { 0x36019064 };	/* VSafe5V @ 1.0A */
static const u32 default_snk_caps[] = { 0x26019096 };	/* VSafe5V @ 1.5A */
...
static void handle_state_snk_wait_for_capabilities(struct usbpd *pd,struct rx_msg *rx_msg);
static void handle_state_prs_snk_src_source_on(struct usbpd *pd,struct rx_msg *rx_msg);
...
enum plug_orientation usbpd_get_plug_orientation(struct usbpd *pd)
{
	...
	ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION, &val);
	if (ret)
...
static unsigned int get_connector_type(struct usbpd *pd)
{
	...
	ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_CONNECTOR_TYPE, &val);
	if (ret) {
...
static inline void start_usb_peripheral(struct usbpd *pd)
{
	...
	val.intval = (cc == ORIENTATION_CC2);
	extcon_set_property(pd->extcon, EXTCON_USB,	EXTCON_PROP_USB_TYPEC_POLARITY, val);
	...
	extcon_set_property(pd->extcon, EXTCON_USB,	EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT, val);
...
static void start_usb_peripheral_work(struct work_struct *w)
{
	...
	typec_set_pwr_opmode(pd->typec_port,pd->typec_mode - POWER_SUPPLY_TYPEC_SOURCE_DEFAULT);
	if (!pd->partner) {
 ...
 */
static int usbpd_release_ss_lane(struct usbpd *pd,	struct usbpd_svid_handler *hdlr)
{
	int ret = 0;
	 ...
	 */
	if (pd->ss_lane_svid) {
		usbpd_dbg(&pd->dev, "-EBUSY: ss_lanes are already used by(%d)",	pd->ss_lane_svid);
		ret = -EBUSY;
		goto err_exit;
	}
...
//发送消息
static int pd_send_msg(struct usbpd *pd, u8 msg_type, const u32 *data,	size_t num_data, enum pd_sop_type sop)
{
	unsigned long flags;
	int ret;
	u16 hdr;
	...
	if (sop == SOP_MSG)
		hdr = PD_MSG_HDR(msg_type, pd->current_dr, pd->current_pr,	pd->tx_msgid[sop], num_data, pd->spec_rev);
	else
		/* sending SOP'/SOP'' to a cable, PR/DR fields should be 0 */
		hdr = PD_MSG_HDR(msg_type, 0, 0, pd->tx_msgid[sop], num_data,pd->spec_rev);
	...
	if (ret) {
		if (pd->pd_connected)
			usbpd_err(&pd->dev, "Error sending %s: %d\n",	msg_to_string(msg_type, num_data,false),ret);
		return ret;
	}
...
//发送扩展消息
static int pd_send_ext_msg(struct usbpd *pd, u8 msg_type,	const u8 *data, size_t data_len, enum pd_sop_type sop)
{
	int ret;
	...
	do {
		...
		chunk_len = min_t(size_t, len_remain,	PD_MAX_EXT_MSG_LEGACY_LEN);
		...
		hdr = PD_MSG_HDR(msg_type, pd->current_dr, pd->current_pr,	pd->tx_msgid[sop], num_objs, pd->spec_rev) |PD_MSG_HDR_EXTENDED;
		ret = pd_phy_write(hdr, chunked_payload,num_objs * sizeof(u32), sop);
		if (ret) {
			usbpd_err(&pd->dev, "Error sending %s: %d\n",usbpd_ext_msg_strings[msg_type],ret);
			return ret;
		}
		...
		if (len_remain &&
			!wait_for_completion_timeout(&pd->tx_chunk_request,	msecs_to_jiffies(SENDER_RESPONSE_TIME))) {
...
static int pd_select_pdo(struct usbpd *pd, int pdo_pos, int uv, int ua)
{
	...
	usbpd_dbg(&pd->dev, "pdo#%d 0x%08x, %d uV %d uA\n", pdo_pos, pdo, uv, ua);
	type = PD_SRC_PDO_TYPE(pdo);
	if (type == PD_SRC_PDO_TYPE_FIXED) {
		curr = max_current = PD_SRC_PDO_FIXED_MAX_CURR(pdo) * 10;

		/* examine source capabilities */
		for (i = 0; i < pd->num_sink_caps; i++) {
			if (PD_SNK_PDO_FIXED_VOLTAGE(pd->sink_caps[i]) ==	PD_SRC_PDO_FIXED_VOLTAGE(pdo)) {
				max_current = min_t(int, max_current,	PD_SNK_PDO_FIXED_MAX_CURR(pd->sink_caps[i]) * 10);
			}
		}
		...
		curr = min(curr, max_current);

		pd->requested_voltage =	PD_SRC_PDO_FIXED_VOLTAGE(pdo) * 50 * 1000;
		usbpd_dbg(&pd->dev, "request voltage %d current %d\n",	pd->requested_voltage / 1000, curr);
...
{
	struct pd_request_chunk *req =	container_of(w, struct pd_request_chunk, w);
	struct usbpd *pd = req->pd;
	unsigned long flags;
	int ret;
	u8 payload[4] = {0}; /* ext_hdr + padding */
	u16 hdr = PD_MSG_HDR(req->msg_type, pd->current_dr, pd->current_pr,	pd->tx_msgid[req->sop], 1, pd->spec_rev)
		| PD_MSG_HDR_EXTENDED;
...
static struct rx_msg *pd_ext_msg_received(struct usbpd *pd, u16 header, u8 *buf,size_t len, enum pd_sop_type sop)
{
	...
	if (PD_MSG_EXT_HDR_REQ_CHUNK(ext_hdr)) {
		if (PD_MSG_EXT_HDR_DATA_SIZE(ext_hdr) ||
			PD_MSG_EXT_HDR_CHUNK_NUM(ext_hdr) !=pd->next_tx_chunk) {
	...
	if (!chunk_num) {
		...
		rx_msg = kzalloc(sizeof(*rx_msg) +	PD_MSG_EXT_HDR_DATA_SIZE(ext_hdr),	GFP_ATOMIC);
		if (!rx_msg)
			return NULL;
	 ...
	 */
	bytes_to_copy =	min((rx_msg->data_len - chunk_num * PD_MAX_EXT_MSG_LEGACY_LEN),	PD_MAX_EXT_MSG_LEGACY_LEN);
	...
	memcpy(rx_msg->payload + chunk_num * PD_MAX_EXT_MSG_LEGACY_LEN, buf + 2,bytes_to_copy);

	/* request next chunk? */
	if ((rx_msg->data_len - chunk_num * PD_MAX_EXT_MSG_LEGACY_LEN) >PD_MAX_EXT_MSG_LEGACY_LEN) {
...
static void phy_msg_received(struct usbpd *pd, enum pd_sop_type sop,u8 *buf, size_t len)
{
	struct rx_msg *rx_msg;
	unsigned long flags;
	u16 header;
	...
	if (PD_MSG_HDR_ID(header) == pd->rx_msgid[sop] &&	PD_MSG_HDR_TYPE(header) != MSG_SOFT_RESET) {
	...
	usbpd_dbg(&pd->dev, "%s type(%d) num_objs(%d)\n", msg_to_string(msg_type, num_objs, PD_MSG_HDR_IS_EXTENDED(header)), msg_type, num_objs);
...
static bool in_src_ams(struct usbpd *pd)
{
	...
	power_supply_get_property(pd->usb_psy, POWER_SUPPLY_PROP_TYPEC_SRC_RP, &val);
...
int usbpd_send_svdm(struct usbpd *pd, u16 svid, u8 cmd, enum usbpd_svdm_cmd_type cmd_type, int obj_pos, const u32 *vdos, int num_vdos)
{
...
static void handle_vdm_resp_ack(struct usbpd *pd, u32 *vdos, u8 num_vdos, u16 vdm_hdr)
{
	int ret, i;
	u16 svid, *psvid;
	...
	case USBPD_SVDM_DISCOVER_IDENTITY:
		...
		usbpd_send_svdm(pd, USBPD_SID, USBPD_SVDM_DISCOVER_SVIDS, SVDM_CMD_TYPE_INITIATOR, 0, NULL, 0);
		break;
	...
	case USBPD_SVDM_DISCOVER_SVIDS:
		...
		if (!pd->discovered_svids) {
			...
			pd->discovered_svids = kcalloc(pd->num_svids, sizeof(u16), GFP_KERNEL);
			if (!pd->discovered_svids)
				break;
			...
			ptr = krealloc(pd->discovered_svids, newsize, GFP_KERNEL);
			if (!ptr)
				break;
...
{
	int ret;
	u32 vdm_hdr = rx_msg->data_len >= sizeof(u32) ? ((u32 *)rx_msg->payload)[0] : 0;
	...
	case SVDM_CMD_TYPE_INITIATOR:
		if (cmd != USBPD_SVDM_ATTENTION) {
			if (pd->spec_rev == USBPD_REV_30) {
				...
				usbpd_send_svdm(pd, svid, cmd, SVDM_CMD_TYPE_RESP_NAK, SVDM_HDR_OBJ_POS(vdm_hdr), NULL, 0);
			}
		}
		break;
...
static void handle_vdm_tx(struct usbpd *pd, enum pd_sop_type sop_type)
{
	...
	ret = pd_send_msg(pd, MSG_VDM, pd->vdm_tx->data, pd->vdm_tx->size, sop_type);
	if (ret) {
...
static void handle_get_src_cap_extended(struct usbpd *pd)
{
	...
	ret = pd_send_ext_msg(pd, MSG_SOURCE_CAPABILITIES_EXTENDED, (u8 *)&caps, sizeof(caps), SOP_MSG);
	if (ret)
		usbpd_set_state(pd, PE_SEND_SOFT_RESET);
}
...
static void handle_get_battery_cap(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	bcdb.last_full = ((pd->bms_charge_full / 1000) *
			(pd->bat_voltage_max / 1000)) / 100000;
send:
	ret = pd_send_ext_msg(pd, MSG_BATTERY_CAPABILITIES, (u8 *)&bcdb, sizeof(bcdb), SOP_MSG);
	if (ret)
		usbpd_set_state(pd, PE_SEND_SOFT_RESET);
}
...
static void handle_get_battery_status(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	ret = power_supply_get_property(pd->bat_psy, POWER_SUPPLY_PROP_PRESENT, &val);
	if (ret || !val.intval)
		goto send;

	bsdo |= BIT(9);

	ret = power_supply_get_property(pd->bat_psy, POWER_SUPPLY_PROP_STATUS, &val);
	if (!ret) {
		switch (val.intval) {
...
static void dr_swap(struct usbpd *pd)
{
	...
	if (pd->current_dr == DR_DFP) {
		...
		usbpd_send_svdm(pd, USBPD_SID, USBPD_SVDM_DISCOVER_IDENTITY, SVDM_CMD_TYPE_INITIATOR, 0, NULL, 0);
	}
}
...
static int usbpd_startup_common(struct usbpd *pd, struct pd_phy_params *phy_params)
{
	int ret = 0;
...
static void handle_state_src_startup_wait_for_vdm_resp(struct usbpd *pd, struct rx_msg *rx_msg)
{
	int ms;
...
static void handle_state_src_send_capabilities(struct usbpd *pd, struct rx_msg *rx_msg)
{
	int ret;
	union power_supply_propval val = {0};

	ret = pd_send_msg(pd, MSG_SOURCE_CAPABILITIES, default_src_caps, ARRAY_SIZE(default_src_caps), SOP_MSG);
	if (ret) {
		if (pd->pd_connected) {
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
			return;
...
static void handle_state_src_send_capabilities_wait(struct usbpd *pd, struct rx_msg *rx_msg)
{
...
static void enter_state_src_negotiate_capability(struct usbpd *pd)
{
	...
	usleep_range(SRC_TRANSITION_TIME * USEC_PER_MSEC, (SRC_TRANSITION_TIME + 5) * USEC_PER_MSEC);
...
static void enter_state_src_ready(struct usbpd *pd)
{
	 ...
	 */
	if (pd->current_dr == DR_DFP && !pd->peer_usb_comm && !pd->in_explicit_contract)
		stop_usb_host(pd);
...
static void handle_state_src_ready(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (IS_CTRL(rx_msg, MSG_GET_SOURCE_CAP)) {
		...
		ret = pd_send_msg(pd, MSG_SINK_CAPABILITIES, pd->sink_caps, pd->num_sink_caps, SOP_MSG);
		if (ret)
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
		...
		ret = pd_send_msg(pd, pd->spec_rev == USBPD_REV_30 ?
				MSG_NOT_SUPPORTED : MSG_REJECT, NULL, 0, SOP_MSG);
		if (ret)
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
		return;
...
static void handle_state_soft_reset(struct usbpd *pd, struct rx_msg *rx_msg)
{
	int ret;
...
static void handle_state_src_transition_to_default(struct usbpd *pd, struct rx_msg *rx_msg)
{
	if (pd->vconn_enabled)
		regulator_disable(pd->vconn);
	pd->vconn_enabled = false;
...
static void enter_state_snk_startup(struct usbpd *pd)
{
	...
	struct pd_phy_params phy_params = {
		...
		.msg_rx_cb		= phy_msg_received,
		.shutdown_cb		= phy_shutdown,
		.frame_filter_val	= FRAME_FILTER_EN_SOP | FRAME_FILTER_EN_HARD_RESET,
	};
	union power_supply_propval val = {0};
...
static void handle_state_snk_wait_for_capabilities(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (IS_DATA(rx_msg, MSG_SOURCE_CAPABILITIES)) {
		...
		memcpy(&pd->received_pdos, rx_msg->payload,	min_t(size_t, rx_msg->data_len,	sizeof(pd->received_pdos)));
		pd->src_cap_id++;
...
static void handle_state_snk_select_capability(struct usbpd *pd,struct rx_msg *rx_msg)
{
	int ret;
	union power_supply_propval val = {0};
	...
	if (IS_CTRL(rx_msg, MSG_ACCEPT)) {
		...
		bool same_pps = (pd->selected_pdo == pd->requested_pdo)
			&& (PD_SRC_PDO_TYPE(pdo) ==	PD_SRC_PDO_TYPE_AUGMENTED);
	...
	} else if (IS_CTRL(rx_msg, MSG_REJECT) || IS_CTRL(rx_msg, MSG_WAIT)) {
		if (pd->in_explicit_contract)
...
static void handle_state_snk_transition_sink(struct usbpd *pd, struct rx_msg *rx_msg)
{
	union power_supply_propval val = {0};
...
static bool handle_data_snk_ready(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	case MSG_BATTERY_STATUS:
		...
		memcpy(&pd->battery_sts_dobj, rx_msg->payload,sizeof(pd->battery_sts_dobj));
		complete(&pd->is_ready);
		break;
	default:
		return false;
...
static bool handle_ext_snk_ready(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	case MSG_SOURCE_CAPABILITIES_EXTENDED:
		...
		memcpy(&pd->src_cap_ext_db, rx_msg->payload,sizeof(pd->src_cap_ext_db));
		complete(&pd->is_ready);
		break;
	case MSG_PPS_STATUS:
		...
		memcpy(&pd->pps_status_db, rx_msg->payload, sizeof(pd->pps_status_db));
		complete(&pd->is_ready);
		break;
	case MSG_STATUS:
		...
		memcpy(&pd->status_db, rx_msg->payload,	sizeof(pd->status_db));
		kobject_uevent(&pd->dev.kobj, KOBJ_CHANGE);
		break;
	case MSG_BATTERY_CAPABILITIES:
		...
		memcpy(&pd->battery_cap_db, rx_msg->payload,sizeof(pd->battery_cap_db));
		complete(&pd->is_ready);
		break;
...
static void handle_snk_ready_tx(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (pd->send_get_src_cap_ext) {
		...
		ret = pd_send_msg(pd, MSG_GET_SOURCE_CAP_EXTENDED, NULL, 0, SOP_MSG);
		if (ret) {
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
			return;
		}
		...
		ret = pd_send_msg(pd, MSG_GET_PPS_STATUS, NULL,	0, SOP_MSG);
		if (ret) {
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
			return;
		}
		...
		ret = pd_send_ext_msg(pd, MSG_GET_BATTERY_CAP,	&pd->get_battery_cap_db, 1, SOP_MSG);
		if (ret) {
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
			return;
		}
		...
		ret = pd_send_ext_msg(pd, MSG_GET_BATTERY_STATUS,&pd->get_battery_status_db, 1, SOP_MSG);
		if (ret) {
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
			return;
		}
...
static void handle_state_snk_ready(struct usbpd *pd, struct rx_msg *rx_msg)
{
	...
	if (rx_msg && !PD_MSG_HDR_COUNT(rx_msg->hdr) &&	handle_ctrl_snk_ready(pd, rx_msg)) {
		return;
	} else if (rx_msg && !PD_MSG_HDR_IS_EXTENDED(rx_msg->hdr) && handle_data_snk_ready(pd, rx_msg)) {
		return;
	} else if (rx_msg && PD_MSG_HDR_IS_EXTENDED(rx_msg->hdr) && handle_ext_snk_ready(pd, rx_msg)) {
		/* continue to handle tx */
	} else if (rx_msg && !IS_CTRL(rx_msg, MSG_NOT_SUPPORTED)) {
		usbpd_dbg(&pd->dev, "Unsupported message\n");
		ret = pd_send_msg(pd, pd->spec_rev == USBPD_REV_30 ?MSG_NOT_SUPPORTED : MSG_REJECT,	NULL, 0, SOP_MSG);
		if (ret)
			usbpd_set_state(pd, PE_SEND_SOFT_RESET);
		return;
	}
...
static void handle_state_snk_transition_to_default(struct usbpd *pd, struct rx_msg *rx_msg)
{
	usbpd_set_state(pd, PE_SNK_STARTUP);
}

static void handle_state_drs_send_dr_swap(struct usbpd *pd, struct rx_msg *rx_msg)
{
	if (IS_CTRL(rx_msg, MSG_ACCEPT))
		dr_swap(pd);

	usbpd_set_state(pd, pd->current_pr == PR_SRC ?	PE_SRC_READY : PE_SNK_READY);
}

static void handle_state_prs_snk_src_send_swap(struct usbpd *pd, struct rx_msg *rx_msg)
{
	if (!IS_CTRL(rx_msg, MSG_ACCEPT)) {
...
static void handle_state_prs_snk_src_transition_to_off(struct usbpd *pd,struct rx_msg *rx_msg)
{
	if (!IS_CTRL(rx_msg, MSG_PS_RDY)) {
...
static void handle_state_prs_src_snk_send_swap(struct usbpd *pd,struct rx_msg *rx_msg)
{
	if (!IS_CTRL(rx_msg, MSG_ACCEPT)) {
...
static void handle_state_prs_src_snk_transition_to_off(struct usbpd *pd,struct rx_msg *rx_msg)
{
	int ret;
...
static void handle_state_prs_src_snk_wait_source_on(struct usbpd *pd,struct rx_msg *rx_msg)
{
	if (IS_CTRL(rx_msg, MSG_PS_RDY))
		usbpd_set_state(pd, PE_SNK_STARTUP);
	else
...
static void enter_state_send_soft_reset(struct usbpd *pd)
{
	...
	if (ret) {
		usbpd_set_state(pd, pd->current_pr == PR_SRC ?	PE_SRC_HARD_RESET : PE_SNK_HARD_RESET);
		return;
	}
...
static void handle_state_send_soft_reset(struct usbpd *pd,struct rx_msg *rx_msg)
{
	if (IS_CTRL(rx_msg, MSG_ACCEPT)) {
		usbpd_set_state(pd, pd->current_pr == PR_SRC ?	PE_SRC_SEND_CAPABILITIES :	PE_SNK_WAIT_FOR_CAPABILITIES);
	} else {
...
static void handle_state_vcs_wait_for_vconn(struct usbpd *pd,struct rx_msg *rx_msg)
{
	if (IS_CTRL(rx_msg, MSG_PS_RDY)) {
		/*
		...
		pd->current_state = pd->current_pr == PR_SRC ?	PE_SRC_READY : PE_SNK_READY;
	} else {
		/* timed out; go to hard reset */
		usbpd_set_state(pd, pd->current_pr == PR_SRC ?	PE_SRC_HARD_RESET : PE_SNK_HARD_RESET);
	}
...
static void usbpd_set_state(struct usbpd *pd, enum usbpd_state next_state)
{
	...
	usbpd_dbg(&pd->dev, "%s -> %s\n",usbpd_state_strings[pd->current_state],usbpd_state_strings[next_state]);

	pd->current_state = next_state;

	if (pd->current_state < PE_MAX_STATES && state_handlers[pd->current_state].enter_state)
...
static void handle_hard_reset(struct usbpd *pd)
{
	...
	if (pd->current_pr == PR_SINK) {
		...
		s64 delta = ktime_ms_delta(ktime_get(),	pd->hard_reset_recvd_time);
		pd->current_state = PE_SRC_TRANSITION_TO_DEFAULT;
...
static void usbpd_sm(struct work_struct *w)
{
	...
	if (pd->current_pr == PR_NONE) {
		if (pd->current_state == PE_UNKNOWN &&	pd->current_dr == DR_NONE)
			goto sm_done;
	...
	if (pd->current_state < PE_MAX_STATES && state_handlers[pd->current_state].handle_state)
...
static int usbpd_process_typec_mode(struct usbpd *pd, enum power_supply_typec_mode typec_mode)
{
	...
	case POWER_SUPPLY_TYPEC_SINK:
		usbpd_info(&pd->dev, "Type-C Sink%s connected\n",typec_mode == POWER_SUPPLY_TYPEC_SINK ? "" : " (powered)");
...
static int psy_changed(struct notifier_block *nb, unsigned long evt, void *ptr)
{
	...
	ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_TYPEC_MODE, &val);
	if (ret) {
	...
	ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_PE_START, &val);
	if (ret) {
	...
	if (!val.intval && !pd->pd_connected &&	typec_mode >= POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) {
		ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_REAL_TYPE, &val);
		if (ret) {
	...
	ret = power_supply_get_property(pd->usb_psy,POWER_SUPPLY_PROP_PRESENT, &val);
	if (ret) {
	 ...
	 */
	if (typec_mode && ((!pd->vbus_present &&pd->current_state == PE_SNK_TRANSITION_TO_DEFAULT) ||
...
static int usbpd_do_swap(struct usbpd *pd, bool dr_swap, unsigned int timeout_ms)
{
	if (pd->current_state != PE_SRC_READY && pd->current_state != PE_SNK_READY) {
	...
	if (!wait_for_completion_timeout(&pd->is_ready,	msecs_to_jiffies(timeout_ms))) {
...
static int usbpd_typec_dr_set(const struct typec_capability *cap,enum typec_data_role role)
{
...
static int usbpd_typec_pr_set(const struct typec_capability *cap,enum typec_role role)
{
	struct usbpd *pd = container_of(cap, struct usbpd, typec_caps);
	bool do_swap = false;
	int ret;
...
static int usbpd_typec_port_type_set(const struct typec_capability *cap,enum typec_port_type type)
{
	struct usbpd *pd = container_of(cap, struct usbpd, typec_caps);
...
//通过uevent机制发送消息给应用层，添加了很多的环境变量
static int usbpd_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	int i;

	add_uevent_var(env, "DATA_ROLE=%s", pd->current_dr == DR_DFP ?	"dfp" : "ufp");
	...
	if (pd->current_pr == PR_SINK) {
		...
		for (i = 0; i < ARRAY_SIZE(pd->received_pdos); i++)
			add_uevent_var(env, "PDO%d=%08x", i,pd->received_pdos[i]);
		...
		for (i = 0; i < ARRAY_SIZE(default_src_caps); i++)
			add_uevent_var(env, "PDO%d=%08x", i,default_src_caps[i]);
	}

	add_uevent_var(env, "RDO=%08x", pd->rdo);
	add_uevent_var(env, "CONTRACT=%s", pd->in_explicit_contract ?"explicit" : "implicit");
	add_uevent_var(env, "ALT_MODE=%d", pd->vdm_state == MODE_ENTERED);

	add_uevent_var(env, "SDB=%02x %02x %02x %02x %02x", pd->status_db[0], pd->status_db[1], pd->status_db[2], pd->status_db[3], pd->status_db[4]);
...
static ssize_t current_pr_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	const char *pr = "none";
...
static ssize_t initial_pr_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	const char *pr = "none";
...
static ssize_t current_dr_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	const char *dr = "none";
...
static ssize_t initial_dr_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	const char *dr = "none";
...
static ssize_t src_cap_id_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
...
static ssize_t pdo_h_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	int i;
	ssize_t cnt = 0;
	...
	for (i = 0; i < ARRAY_SIZE(pd->received_pdos); i++) {
		...
		if (PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_FIXED) {
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt, "\tFixed supply\n"
					"\tDual-Role Power:%d\n"
					...
					"\tPeak Current:%d\n"
					"\tVoltage:%d (mV)\n"
					"\tMax Current:%d (mA)\n", PD_SRC_PDO_FIXED_PR_SWAP(pdo), PD_SRC_PDO_FIXED_USB_SUSP(pdo), PD_SRC_PDO_FIXED_EXT_POWERED(pdo), PD_SRC_PDO_FIXED_USB_COMM(pdo), PD_SRC_PDO_FIXED_DR_SWAP(pdo), PD_SRC_PDO_FIXED_PEAK_CURR(pdo), PD_SRC_PDO_FIXED_VOLTAGE(pdo) * 50, PD_SRC_PDO_FIXED_MAX_CURR(pdo) * 10);
		} else if (PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_BATTERY) {
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt, "\tBattery supply\n"
					"\tMax Voltage:%d (mV)\n"
					"\tMin Voltage:%d (mV)\n"
					"\tMax Power:%d (mW)\n", PD_SRC_PDO_VAR_BATT_MAX_VOLT(pdo) * 50, PD_SRC_PDO_VAR_BATT_MIN_VOLT(pdo) * 50, PD_SRC_PDO_VAR_BATT_MAX(pdo) * 250);
		} else if (PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_VARIABLE) {
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt, "\tVariable supply\n"
					"\tMax Voltage:%d (mV)\n"
					"\tMin Voltage:%d (mV)\n"
					"\tMax Current:%d (mA)\n", PD_SRC_PDO_VAR_BATT_MAX_VOLT(pdo) * 50, PD_SRC_PDO_VAR_BATT_MIN_VOLT(pdo) * 50, PD_SRC_PDO_VAR_BATT_MAX(pdo) * 10);
		} else if (PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_AUGMENTED) {
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt, "\tProgrammable Power supply\n"
					"\tMax Voltage:%d (mV)\n"
					"\tMin Voltage:%d (mV)\n"
					"\tMax Current:%d (mA)\n", PD_APDO_MAX_VOLT(pdo) * 100, PD_APDO_MIN_VOLT(pdo) * 100, PD_APDO_MAX_CURR(pdo) * 50);
		} else {
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt,"Invalid PDO\n");
		}
...
static ssize_t select_pdo_store(struct device *dev,	struct device_attribute *attr, const char *buf, size_t size)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	int src_cap_id;
	int pdo, uv = 0, ua = 0;
	...
	if (pd->selected_pdo != pd->requested_pdo ||pd->current_voltage != pd->requested_voltage) {
...
static ssize_t select_pdo_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
...
static ssize_t rdo_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
...
static ssize_t rdo_h_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	...
	len = scnprintf(buf, PAGE_SIZE, "Request Data Object\n"
			...
			"\tUSB Communications Capable:%d\n"
			"\tNo USB Suspend:%d\n", PD_RDO_OBJ_POS(pd->rdo), PD_RDO_GIVEBACK(pd->rdo), PD_RDO_MISMATCH(pd->rdo), PD_RDO_USB_COMM(pd->rdo), PD_RDO_NO_USB_SUSP(pd->rdo));

	switch (type) {
	case PD_SRC_PDO_TYPE_FIXED:
	case PD_SRC_PDO_TYPE_VARIABLE:
		len += scnprintf(buf + len, PAGE_SIZE - len, "(Fixed/Variable)\n"
				"\tOperating Current:%d (mA)\n"
				"\t%s Current:%d (mA)\n", PD_RDO_FIXED_CURR(pd->rdo) * 10, PD_RDO_GIVEBACK(pd->rdo) ? "Min" : "Max", PD_RDO_FIXED_CURR_MINMAX(pd->rdo) * 10);
		break;

	case PD_SRC_PDO_TYPE_BATTERY:
		len += scnprintf(buf + len, PAGE_SIZE - len, "(Battery)\n"
				"\tOperating Power:%d (mW)\n"
				"\t%s Power:%d (mW)\n", PD_RDO_FIXED_CURR(pd->rdo) * 250, PD_RDO_GIVEBACK(pd->rdo) ? "Min" : "Max", PD_RDO_FIXED_CURR_MINMAX(pd->rdo) * 250);
		break;

	case PD_SRC_PDO_TYPE_AUGMENTED:
		len += scnprintf(buf + len, PAGE_SIZE - len, "(Programmable)\n"
				"\tOutput Voltage:%d (mV)\n"
				"\tOperating Current:%d (mA)\n", PD_RDO_PROG_VOLTAGE(pd->rdo) * 20, PD_RDO_PROG_CURR(pd->rdo) * 50);
		break;
	}
...
static ssize_t hard_reset_store(struct device *dev,	struct device_attribute *attr, const char *buf, size_t size)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	u32 val = 0;
...
static ssize_t get_src_cap_ext_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	int i, ret, len = 0;
	struct usbpd *pd = dev_get_drvdata(dev);
	...
	for (i = 0; i < PD_SRC_CAP_EXT_DB_LEN; i++)
		len += scnprintf(buf + len, PAGE_SIZE - len, "%s0x%02x",i ? " " : "", pd->src_cap_ext_db[i]);
...
static ssize_t get_status_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	int i, ret, len = 0;
	struct usbpd *pd = dev_get_drvdata(dev);
	...
	for (i = 0; i < PD_STATUS_DB_LEN; i++)
		len += scnprintf(buf + len, PAGE_SIZE - len, "%s0x%02x", i ? " " : "", pd->status_db[i]);
...
static ssize_t get_pps_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
	struct usbpd *pd = dev_get_drvdata(dev);
...
static ssize_t get_battery_cap_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	u32 val;
	int ret;

	if (pd->spec_rev == USBPD_REV_20 || kstrtou32(buf, 0, &val) || val != 1) {
...
static ssize_t get_battery_cap_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	int i, len = 0;
	struct usbpd *pd = dev_get_drvdata(dev);
	...
	for (i = 0; i < PD_BATTERY_CAP_DB_LEN; i++)
		len += scnprintf(buf + len, PAGE_SIZE - len, "%s0x%02x", i ? " " : "", pd->battery_cap_db[i]);
...
static ssize_t get_battery_status_store(struct device *dev,	struct device_attribute *attr, const char *buf, size_t size)
{
	struct usbpd *pd = dev_get_drvdata(dev);
	u32 val;
	int ret;
...
static ssize_t get_battery_status_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
...
struct usbpd *devm_usbpd_get_by_phandle(struct device *dev, const char *phandle)
{
	...
	pd_dev = class_find_device(&usbpd_class, NULL, &pdev->dev,	match_usbpd_device);
	if (!pd_dev) {
...
struct usbpd *usbpd_create(struct device *parent)
{
	...
	extcon_set_property_capability(pd->extcon, EXTCON_USB,	EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(pd->extcon, EXTCON_USB,	EXTCON_PROP_USB_SS);
	extcon_set_property_capability(pd->extcon, EXTCON_USB,	EXTCON_PROP_USB_TYPEC_MED_HIGH_CURRENT);
	extcon_set_property_capability(pd->extcon, EXTCON_USB_HOST,	EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(pd->extcon, EXTCON_USB_HOST,	EXTCON_PROP_USB_SS);

	pd->num_sink_caps = device_property_read_u32_array(parent,	"qcom,default-sink-caps", NULL, 0);
	if (pd->num_sink_caps > 0) {
		...
		ret = device_property_read_u32_array(parent,"qcom,default-sink-caps", sink_caps,pd->num_sink_caps);
		if (ret) {
		...
		for (i = 0; i < pd->num_sink_caps; i++) {
			...
			pd->sink_caps[i] =	PD_SNK_PDO_FIXED(0, 0, 0, 0, 0, v, c);
		}

		/* First PDO includes additional capabilities */
		pd->sink_caps[0] |= PD_SNK_PDO_FIXED(1, 0, 0, 1, 1, 0, 0);
	} else {
		memcpy(pd->sink_caps, default_snk_caps,	sizeof(default_snk_caps));
